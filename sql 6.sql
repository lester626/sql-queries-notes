...
select avg(price) as 'avg value for prices' from song where artistid=1;
select min(price) as 'cheapest' from song where artistid=1;
select max(price) as 'most expensive' from song where artistid=1;
select count(*) as 'amount of songs' from song;
select sum(price) as 'sum of prices' from song where artistid=1;

select artistid, (price) as 'cheapest', max(price) as 'most expensive' from song group by (artistid);
